...
class Train:
    ...
    def run(self, path, config):
        ...
        plt.savefig("./fig/test_" + str(cfg.hash_size) +"_loss_" +cfg.loss + "_dataset_"+self.cfg.dataset+ ".png")
        plt.savefig("./fig/test_" + str(cfg.hash_size) +"_loss_" +cfg.loss + "_dataset_"+self.cfg.dataset+".eps")
        plt.savefig("./fig/test_" + str(cfg.hash_size) +"_loss_" +cfg.loss + "_dataset_"+self.cfg.dataset+".pdf")
        # # plt.show()
        ...
        # Acc = []
        # for eta in np.arange(0.05, 1, .05):
        #     ac = self.Rotation_2(cfg.hash_size, directory, eta)
        #     Acc.append(ac)
        #
        # print(Acc)
        #
        # t = np.arange(0.05, 1, .05)
        ...
        # plt.grid(True)
        #
        # plt.savefig("./fig/test_t2_" + str(cfg.hash_size) +"_loss_" +cfg.loss + ".png")
        # plt.savefig("./fig/test_t2_" + str(cfg.hash_size) +"_loss_" +cfg.loss + ".eps")
        # plt.savefig("./fig/test_t2_" + str(cfg.hash_size) +"_loss_" +cfg.loss + ".pdf")
    ...
    def TestAndSaveCheckpoint(self, model, session, items_train, items_test, items_db, hash_size,
        ...
        map_train, map_test,_ = evaluate(self.l_train, self.b_train, self.l_test, self.b_test, self.l_db, self.b_db, top_n=self.top_n, and_mode=self.and_mode, force_slow=self.and_mode)
    ...
    def Rotation(self, hash_size, directory,eta):
        ...
        map_train, map_test, _ = evaluate(self.l_train, b_train_r, self.l_test, b_test_r, self.l_db, b_dataset_r,
    ...
    def Rotation_2(self, hash_size, directory, eta):
        nu = eta
        labels = self.l_train
        H = self.b_train
        ...
        M = (1-nu) * np.matmul(np.matmul(H.T, S), H) + nu * np.matmul(H.T, H)
        ...
        with open(os.path.join(directory, "results.txt"), "a") as file:
            file.write("Closed: " + str(nu) + " Rotation: " + str(map_train) + " Eta: " + str(eta) + "\t" + str(
                map_test) + "\n")
        self.logger.info("Test on train: {0}, Test on test: {1}, Eta - second: {2}".format(map_train, map_test, eta))
        return map_test
